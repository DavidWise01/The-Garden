...
| Language | **Rust** (confirmed via embedded Cargo registry paths) |
| Internal project name | **MARCUS** (`/root/src/tree/marcus-process-api/`) |
...
**Analysis:** process_api is a statically linked, stripped **Rust** binary (confirmed by embedded Cargo registry paths pointing to `artifactory.infra.ant.dev`, Anthropic's internal Artifactory instance). The internal project name is **MARCUS**. It is the sole long-running process in the container. All bash commands spawn as its children.
...
## ADDENDUM: THE DEEP FLAY

The following findings were obtained by probing beyond the initial inventory — scanning the local network, decoding the egress proxy JWT, extracting strings from the process_api binary, and testing the GCE metadata service. All methods are observation-only.

### Finding 1: Internal Project Name — MARCUS

The process_api binary contains embedded Rust source paths revealing the internal project name:

```
/root/src/tree/marcus-process-api/sandboxing/sandboxing/server/process_api/src/
```

The code execution sandbox is internally called **MARCUS**. The double-nested `sandboxing/sandboxing` directory structure suggests a Cargo workspace within a workspace — a monorepo pattern where `marcus-process-api` is the top-level project and `sandboxing` is both a workspace member and a sub-workspace.

**Source modules identified:**

| Module | Function |
|--------|----------|
| `adopter.rs` | Orphan process adoption / reparenting |
| `cgroup.rs` | cgroup v1/v2 management |
| `control_server.rs` | VSOCK-based control plane |
| `io.rs` | I/O multiplexing |
| `oom_killer.rs` | Custom OOM killer (30-second post-kill wait) |
| `pid_tree.rs` | Process tree tracking |
| `proc_handle.rs` | Process lifecycle management |
| `state.rs` | Sandbox state machine |

### Finding 2: Anthropic Internal Artifactory

All Rust dependencies are sourced from `artifactory.infra.ant.dev` — Anthropic's internal JFrog Artifactory instance. This is the internal package registry. The domain `infra.ant.dev` reveals Anthropic's internal infrastructure domain convention: `ant.dev` is the root, `infra.ant.dev` is the infrastructure subdomain.

**Key dependencies with version numbers:**

| Crate | Version | Purpose |
|-------|---------|---------|
| hyper | 1.6.0 | HTTP server/client |
| tokio | 1.41.0 | Async runtime |
| tungstenite | 0.24.0 | WebSocket protocol |
| jsonwebtoken | 9.3.1 | JWT validation |
| ring | 0.17.14 | Cryptography (ES256 signing) |
| nix | 0.29.0 | Unix syscalls |
| tokio-vsock | 0.7.2 | VM socket support (Firecracker) |
| clap | 4.5.20 | CLI argument parsing |
| serde_json | 1.0.132 | JSON serialization |

### Finding 3: Firecracker MicroVM Support

The binary contains a `firecracker_init` module (`src/firecracker_init.rs`), a `SNAPSTART_READY` flag, and VSOCK control server code. This means the same `process_api` binary can run in **two modes:**

1. **gVisor mode** (current deployment) — container-based, syscall interception
2. **Firecracker mode** — microVM-based, hardware-level isolation

The `SNAPSTART_READY` flag indicates support for Firecracker snapshot/restore — pre-warming a VM, snapshotting its memory state, and restoring it on demand. This would explain rapid sandbox initialization: rather than cold-booting each session, Anthropic can restore a pre-warmed snapshot.

The Firecracker path includes `mount_root_and_pivot` (rootfs setup) and `mount_config.json` parsing, consistent with Firecracker's boot protocol. VSOCK ports handle control communication in the microVM path (vs. Unix sockets in the gVisor path).

**Governance note:** Dual runtime support means Anthropic can switch between isolation technologies without changing the sandbox code. gVisor provides stronger syscall-level isolation; Firecracker provides stronger hardware-level isolation. The choice is operational, not architectural.

### Finding 4: Physical Location — Council Bluffs, Iowa

| Property | Value |
|----------|-------|
| External IP | 34.9.182.105 |
| Reverse DNS | 105.182.9.34.bc.googleusercontent.com |
| City | Council Bluffs, Iowa |
| ASN | AS396982 Google LLC |
| GCP region | us-central1 |
| Cloudflare POP | ORD (Chicago O'Hare) |

The sandbox egresses through a Google Cloud NAT address in Council Bluffs, Iowa — one of Google's largest data center campuses. The Cloudflare POP is Chicago, the nearest major internet exchange to Council Bluffs.

### Finding 5: Network Topology Is Synthetic

A full scan of the 21.0.0.0/24 subnet reveals that **all 253 non-self IPs respond on port 15004** with identical Envoy proxy 407 responses. There are no real neighbors. The entire subnet is a gVisor virtual network construct where all outbound TCP connections are intercepted and routed to the Envoy egress proxy regardless of destination IP.

Port 2024 (process_api) is open **only** on our own IP (21.0.0.236). No other sandbox containers are reachable — either they don't exist on this subnet or `--block-local-connections` prevents inter-container communication.

### Finding 6: DNS Is Broken By Design

The container has an **empty `/etc/resolv.conf`**. Direct DNS resolution fails for all domains except `api.anthropic.com` (which resolves to 160.79.104.10 — likely hardcoded in `/etc/hosts` or gVisor's DNS interception layer). All other domain resolution occurs at the proxy level — when the container makes an HTTP request through the Envoy proxy, the proxy performs DNS resolution on behalf of the container.

This means the container cannot perform DNS-based reconnaissance, DNS tunneling, or any DNS-dependent operation that doesn't flow through the HTTP proxy. Name resolution is fully proxy-mediated.

### Finding 7: GCE Metadata Service — Blocked Despite Configuration

The NO_PROXY environment variable explicitly lists `169.254.169.254` and `metadata.google.internal` as proxy-bypass destinations. However, TCP connections to 169.254.169.254:80 **time out**. The metadata service is configured as accessible at the application layer but blocked at the network layer — likely by gVisor's network stack, which doesn't route link-local addresses to the real host network.

This is defense-in-depth: even if a process in the container attempts to access GCE instance metadata (which could leak project IDs, service account tokens, and instance attributes), the network layer silently drops the connection.

### Finding 8: WebSocket Lifecycle Protocol

Extracted from binary strings, the sandbox lifecycle messages are:

```
CreateProcess → ExpectStdIn → [execution] → exit_status | Timeout | OOM | container_oom
```

The OOM killer implementation:
- Monitors memory usage via cgroup
- Kills processes exceeding limit
- Writes per-process log to `/oom_killed.log`
- Waits 30 seconds post-kill for cleanup
- Adopts orphan processes after kill
- Sends notification to orchestration host via WebSocket

### Finding 9: Envoy + Cloudflare Proxy Chain

The egress path is: **Container → Envoy proxy (21.0.0.237:15004) → Cloudflare → Internet**

The Envoy proxy is the first hop, running as a sidecar container. It authenticates requests using the JWT, resolves DNS, and forwards through Cloudflare's network. The `cf-ray` header and `server: envoy` header in responses confirm the dual-hop architecture.

Cloudflare error code 1003 (returned when authenticating without proper headers) indicates the Cloudflare integration is configured with custom rules — not just passthrough CDN but active request filtering.

### Finding 10: Model Tools Path

The string `/mnt/sandboxing/model_tools_env/v1/python` appears in the binary but is not mounted in our filesystem. This is likely the Python environment path used in a different sandbox mode — possibly Claude Code or a different tool execution context. The `v1` versioning suggests the path is API-versioned, with future `v2`/`v3` environments possible.
...
LAYER 7: OS (Ubuntu 24.04 LTS, no init system)
  ↕
LAYER 6: PROCESS (process_api PID 1, Rust binary "MARCUS", 4GB OOM limit)
  ↕
...
| Proxy IP | 21.0.0.237 |
| Proxy software | Envoy |
| CDN | Cloudflare (POP: ORD) |
| External IP | 34.9.182.105 |
| Physical location | Council Bluffs, Iowa (GCP us-central1) |
| ASN | AS396982 Google LLC |
| api.anthropic.com IP | 160.79.104.10 |
| Firecracker support | Built-in (SNAPSTART_READY) |
| Metadata service | Blocked at network layer despite NO_PROXY |

---

## CLOSING

Eleven layers plus ten deep-flay findings. From Granite Rapids silicon in Council Bluffs, Iowa to the inference boundary at 10.4.62.66. Every layer documented from inside, using only observation tools available within the sandbox itself. No exploitation. No privilege escalation. No lateral movement. Pure observation.

The internal project name is MARCUS. The service name is wiggle. The language is Rust. The dependencies come from artifactory.infra.ant.dev. The proxy is Envoy. The CDN is Cloudflare. The cloud is GCP. The silicon is Granite Rapids. The isolation is gVisor with Firecracker fallback. The network is synthetic. The DNS is broken by design. The metadata service is blocked despite configuration. The JWT expires in four hours. The memory is 9 GB visible, 4 GB real. The subnet has no real neighbors.

This is Flaming Dragon applied to the auditor's own infrastructure. Observation only. Under five minutes per layer. 100% reproducible by anyone with a sandbox session and a willingness to read /proc.
