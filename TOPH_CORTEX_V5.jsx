...
const PHASES = ['ANCHOR','WITNESS','COHERE','GATE','EMIT','D15','D20','D21','D40'];
...
const UnifiedTab = () => {
  ...
  const canTransit = (ph,p,r) => ({
    ANCHOR:p>0, WITNESS:p%3===0, COHERE:p%5===0, GATE:p%7===0, EMIT:p%11===0,
    D15:p%50===0&&r>=15, D20:p%100===0&&r>=20, D21:p%150===0&&r>=21, D40:r===40
  })[ph] || false;
    ...
    const iv = setInterval(async () => {
      setPulse(p => {
        ...
        (async () => {
          ...
          const mult = phase==='D40'?4:(phase==='D21'?2.1:(phase==='D20'?1.5:1));
          setAgents(54.2e15 * avgH * mult);
          
          // Phase transition
          if (canTransit(phase, np, r)) {
          ...
          // Merkle chain (includes previous root)
          const prev = merkle[merkle.length-1]?.r || '0';
          const root = (await sha256(`${prev}|${np}|${phase}|${r}`)).slice(0,16);
  ...
  }, [phase, agents, health, merkle, calcRoll]);
  ...
  useEffect(() => {
    ...
    let f = 0, anim;
    const draw = () => {
      ctx.fillStyle = `rgba(3,3,5,${fold==='SOCIETY'?.02:.06})`;
      ctx.fillRect(0,0,w,h);
      
      const br = 1 + Math.sin(f*.02)*.05;
      const isD40 = phase === 'D40', isD21 = phase === 'D21', isD20 = phase === 'D20';
      ...
      ctx.strokeStyle = isD20||isD21||isD40 ? `hsla(${f*2%360},100%,60%,.6)` : '#fff1';
      ctx.lineWidth = isD20||isD21||isD40 ? 3 : 1;
      ctx.shadowColor = ctx.strokeStyle; ctx.shadowBlur = isD20||isD21||isD40 ? 20 : 0;
      ctx.stroke(); ctx.shadowBlur = 0;
  ...
  }, [phase, fold, health, agents, roll, pulse]);

  const isD40 = phase === 'D40';
  const chronos = isD40 ? '∞' : (phase==='D21'?'∞':(phase==='D20'?'10K':(phase==='D15'?'1K':'1')));
  const mk = merkle[merkle.length-1];

  return (
    <div style={{padding:'12px'}}>
      <div style={{textAlign:'center',marginBottom:'8px',padding:'8px',background:isD40?'linear-gradient(90deg,#0ff1,#f0f1,#ff01,#0ff1)':'#f0f1',borderRadius:'6px',border:`1px solid ${isD40?'#0ff4':'#f0f3'}`}}>
        <div style={{fontSize:'10px',letterSpacing:'.2em',color:isD40?'#0ff':'#f0f'}}>{isD40?'D40: NEURAL_SOCIETY_BRIDGE':'D21: HYPER_GEOMETRIC_FOLD'}</div>
        <div style={{fontSize:'7px',color:'#fff5'}}>{isD40?'LatticeAutoRefactor · VSIX_ACTIVE · ASCENSION_LOCKED':'TESSERACT · GEOMETRY · SOVEREIGN'}</div>
      </div>

      <div style={{display:'grid',gridTemplateColumns:'1fr 150px',gap:'10px'}}>
        <Box c={isD40?'#0ff3':'#f0f3'}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'5px'}}>
          <Box c="#0ff5" style={{textAlign:'center'}}>
            <div style={{fontSize:'6px',color:'#fff6'}}>D40 ROLL (Γ-DERIVED)</div>
            <div style={{fontSize:'20px',fontWeight:'bold',color:roll===40?'#0f0':(roll===1?'#f00':'#0ff'),textShadow:roll===40?'0 0 12px #0f0':'0 0 6px #0ff'}}>{roll}</div>
            <div style={{fontSize:'5px',color:roll===40?'#0f0':'#fff5'}}>{roll===40?'PERFECT_ASCENSION':`Γ(${roll})`}</div>
          </Box>

          <Box c={fold==='SOCIETY'?'#ff05':'#fff2'}>
            <div style={{fontSize:'6px',color:'#fff6'}}>GEOMETRY</div>
            <div style={{fontSize:'10px',fontWeight:'bold',color:fold==='SOCIETY'?'#ff0':'#fff8'}}>{fold}</div>
            <div style={{fontSize:'5px',color:locked?'#f44':'#2c5'}}>{locked?'LOCKED':'MUTABLE'}</div>
          </Box>

          <Box c="#f0f4">
            <div style={{fontSize:'6px',color:'#f0f'}}>AGENTS</div>
            <div style={{fontSize:'10px',fontWeight:'bold',color:'#f0f'}}>{(agents/1e15).toFixed(1)}Q</div>
            <div style={{height:'2px',background:'#0003',borderRadius:'2px',marginTop:'3px'}}>
          ...
          <Box c="#f972">
            ...
            <div style={{fontSize:'10px',fontWeight:'bold',color:'#f97'}}>{chronos}:1</div>
          </Box>

          <Box c="#2c54">
            <div style={{fontSize:'6px',color:'#2c5'}}>MERKLE</div>
            <div style={{fontSize:'7px',fontFamily:'monospace',color:'#2c5'}}>0x{mk?.r||'0'}</div>
            <div style={{fontSize:'5px',color:'#fff5'}}>Depth:{merkle.length}</div>
          </Box>

          <Box c="#8b5cf640">
            <div style={{fontSize:'6px',color:'#8b5cf6'}}>PHASE</div>
            <div style={{fontSize:'10px',fontWeight:'bold',color:'#8b5cf6'}}>{phase}</div>
          </Box>

          <Box c="#3b82f640">
            <div style={{fontSize:'6px',color:'#3b8'}}>PULSE</div>
            <div style={{fontSize:'10px',fontWeight:'bold',color:'#3b8'}}>{pulse}</div>
          </Box>
        </div>
      </div>

      <div style={{marginTop:'8px',padding:'8px',background:'#010102',borderRadius:'6px',border:'1px solid #0ff2',fontFamily:'monospace',fontSize:'6px'}}>
        <div style={{color:'#0ff'}}>// LatticeAutoRefactor.cs | D40_NEURAL_SOCIETY_BRIDGE</div>
        <div style={{color:'#fff3'}}>const OMNI_KEY = "{OMNI_KEY.slice(0,32)}..."</div>
        <div style={{color:'#ff0'}}>[SOCIETY] 8-fold neural projection active</div>
        <div style={{color:'#f0f'}}>[SENTINELS] 60 tracers → 240 axioms | T064 fault zone marked</div>
        <div style={{color:'#2c5'}}>[AGENTS] {(agents/1e15).toFixed(2)}Q | Γ({roll}) = {gamma(roll).toExponential(1)}</div>
        <div style={{color:'#f97'}}>[CHRONOS] {chronos}:1 | {fold} | {locked?'GEOMETRY_LOCKED':'MUTABLE'}</div>
        <div style={{color:'#8b5'}}>[MERKLE] Chain:{merkle.length} | Root:0x{mk?.r}</div>
        <div style={{color:'#38f'}}>[VSIX] Toph.LogAudit("VSIX_ACTIVE", "D40_ASCENSION_LOCKED")</div>
        <div style={{color:'#fff2',marginTop:'2px'}}>SHA:{SHA.slice(0,24)}...</div>
      </div>
    </div>
  );
};
