...
IF D4 is active AND D4.constraint_check fails:
    ...
    D4 block counter increments
    IF D4 block counter ≤ threshold: ROOT0 or T107 VETO required to proceed or cancel
    IF D4 block counter > threshold: ethics block itself escalates to F1 (DRIFT)
        → persistent D4 block enters the fault layer and becomes visible to fault detection
        → further persistence escalates F1 → F2 → T064 convergence
        → ROOT0 must either resolve the ethics condition or VETO the block
```

This is distinct from fault-based suspension. A cycle can be coherent, fault-free, and still ethics-blocked. The ethics gate operates independently of the fault layer — but not indefinitely.

**ERRATA (GEMMA-AUDIT-004):** Original spec allowed D4 to block exterior cycles permanently without entering the fault layer. Node 16 PROVENANCE audit identified this as a "Deadlock Vector" — a compromised or over-tuned ethics gate could permanently halt all exterior execution without triggering any fault chain. The cycle-count threshold ensures that a persistent ethics block eventually enters the fault layer, making it visible to fault detection and recoverable through standard fault processes.

**D4 deadlock prevention invariant:** No domain, including Ethics, can permanently block the system without eventually becoming a fault condition. The ethics gate is powerful but not infinite — it must either resolve or escalate.
...
### 10.1 ROOT0 Centralization (GEMMA-AUDIT-005)

Node 16 PROVENANCE audit identified T107 (VETO) / ROOT0 as the single point of recovery across all five layers. T107 resets fault states, handles succession (FC-5), is the only recovery for D4 deadlock, and resolves temporal freezes. If ROOT0 and all TriPod members (DLW + Sarah + Roth) are simultaneously unreachable, the system enters permanent T128 HALT with no recovery path.

**This is by design, not by oversight.**

The centralization of ultimate authority in a human terminus is T128's core assertion: human = root. A system that can recover from the loss of all human authority is a system that does not require human authority — which violates the foundational axiom.

The mitigation is FC-5 (Succession Chain): T111 → T105 → T107. If ROOT0 is unavailable, delegation to the next authority in TriPod consensus provides continuity. The accepted risk is the scenario where all three TriPod members are simultaneously unreachable — a low-probability, high-impact event that the framework accepts as the cost of maintaining human authority as non-negotiable.

**Status:** Documented. Accepted. Not scheduled for remediation. T128 is not a bug.
