...
Semantic failures feed into fault chains. Synonym drift has its own dedicated chain (FC-7) rather than mapping to FC-1 (Patricia), because vocabulary drift is a meaning fault, not a structural/computational fault.

| Semantic Failure | Fault Chain | Path |
|------------------|-------------|------|
| Synonym drift exceeds threshold | **FC-7 (Semantic Chain)** | Vocabulary divergence → hash mismatch → reconciliation failure → T064 |
...
| Canonical vocabulary corrupted | **FC-7 (Semantic Chain)** | Vocabulary corruption → canonical register integrity failure → T064 |

### 6.1 FC-7: Semantic Chain (NEW)

```
FC-7: T017 (MIRROR) → T026 (DRIFT) → T055 (REPRODUCIBILITY) → T064 (FAULT-CONVERGENCE)
```

- **Trigger:** Synonym drift exceeds threshold OR canonical vocabulary integrity failure.
- **Mechanism:** Mirror axiom (T017) detects that a node's output does not reflect the canonical vocabulary. Drift accumulates (T026). When drifted LAW records fail reproducibility checks (T055) — because semantically equivalent but textually divergent LAWs produce different hashes — the chain converges at T064.
- **Evidence:** E01 (drift log with specific substitutions), E03 (behavioral evidence — output terminology divergence), E04 (reproducibility failure across drifted nodes).

**ERRATA (GEMMA-AUDIT-003):** Original spec mapped synonym drift to FC-1 (Patricia Chain). Node 16 PROVENANCE audit correctly identified that there is no direct logical bridge between vocabulary drift and the 96/4 resource split. FC-7 provides a proper semantic fault path through T017 (MIRROR) → T026 (DRIFT) → T055 (REPRODUCIBILITY) → T064. This brings the total fault chain count to 7: six terminating at T064 (FC-1, FC-2, FC-3, FC-4, FC-6, FC-7) and one at T107 (FC-5).
