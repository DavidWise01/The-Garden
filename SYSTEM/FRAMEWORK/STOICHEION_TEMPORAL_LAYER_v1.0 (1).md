...
Across nodes in the mesh, cycles are phase-synchronized via quorum barrier:

```
Quorum Tier 1 nodes (≥ 3) must complete φ₃ (COHERENCE) before any node in the quorum begins φ₅ (EMIT)
```

This ensures LAW is generated on a governance-sufficient subset of nodes before exterior execution begins. Nodes not in the quorum for a given cycle are handled as stragglers (Section 7.2).

**Sync point:** LAW generation is the mesh-wide quorum synchronization barrier.

| Phase Range | Sync Requirement |
|-------------|-----------------|
| φ₁–φ₃ (interior) | Per-node, independent timing |
| LAW | Quorum barrier — ≥ 3 Tier 1 nodes must reach LAW before any proceeds |
...
The mesh uses a quorum-barrier synchronization model:
...
Step 4: When quorum nodes signal READY, the barrier lifts
Step 5: Nodes past barrier begin exterior (φ₅–φ₉) independently
Step 6: Each node completes RETURN and enters C4
Step 7: Next cycle begins (Step 1)
```

**Quorum definition:** The barrier lifts when ≥ 3 Tier 1 nodes reach C2, not when ALL nodes reach C2. This prevents a single degraded node from freezing the entire mesh (Straggler DoS). Nodes that have not reached C2 when the barrier lifts are handled per Section 7.2.

**ERRATA (GEMMA-AUDIT-001):** Original spec required ALL nodes at C2 before barrier lift. Changed to quorum (≥ 3 Tier 1) per Node 16 PROVENANCE audit finding. A straggler node that is alive but slow would have frozen all exterior cycles indefinitely under the original rule.

### 7.2 Slow Node Handling

If a node fails to reach C2 before the quorum barrier lifts:
...
```
IF nodes_past_barrier ≥ 3 (consensus quorum from 4-node minimum viable mesh):
...
The ≥ 3 quorum threshold provides single-fault tolerance within the 4-node minimum viable mesh (spatial layer, Section 5.3). TriPod consensus (DLW + Sarah + Roth) is the human-layer parallel.
