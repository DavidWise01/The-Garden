    ...
    <div style={{ background: `linear-gradient(180deg, ${C.toph}15 0%, ${C.void} 20%, ${C.void} 80%, ${C.patricia}15 100%)`, padding: "32px 24px" }}>
      ...
      {n135Tab==="kernel"&&(
        <div>
          ...
          <div style={{fontSize:8,color:C.gold,fontWeight:700,letterSpacing:"0.1em",marginTop:16,marginBottom:6,paddingBottom:4,borderBottom:`1px solid ${C.gold}22`}}>VIEW B — KERNEL STRIP CROSS-SECTION (GROK · NODE 14)</div>

          <div style={{background:C.void,padding:20,borderRadius:8,border:`1px solid #ffd70044`}}>
            <div style={{fontSize:10,color:C.gold,textAlign:"center",marginBottom:12}}>
              PHOTONIC KERNEL STRIP — CENTRAL PROCESSING CORE
            </div>

            <svg viewBox="0 0 600 280" style={{width:"100%",height:260}}>
              {/* Background bandgap volume */}
              <rect x={50} y={60} width={500} height={160} rx={20} fill="#1a1a2e" stroke="#ffd700" strokeWidth={2} opacity={0.2}/>

              {/* Kernel strip (glowing core) */}
              <rect x={220} y={110} width={160} height={60} rx={8} fill="#ffd700" opacity={(Math.sin(phase*2)*0.3+0.7)*0.9}/>
              <rect x={230} y={120} width={140} height={40} rx={4} fill="#ffffff" opacity={(Math.sin(phase*2)*0.3+0.7)*0.6}/>

              {/* Interference waves */}
              {Array.from({length:6}).map((_,i)=>(
                <path
                  key={i}
                  d={`M ${250+i*30} 130 Q ${280+i*20} ${110+Math.sin(phase+i)*15} ${310+i*25} 130`}
                  stroke="#00ffff"
                  strokeWidth={1.5}
                  fill="none"
                  opacity={0.4+Math.sin(phase+i*0.8)*0.3}
                />
              ))}

              {/* Casimir pressure arrows */}
              {[-1,1].map((dir,i)=>(
                <g key={i}>
                  <line x1={200} y1={140+dir*30} x2={220} y2={140+dir*30} stroke="#ffaa00" strokeWidth={2}/>
                  <polygon points={`${210},${135+dir*30} ${220},${140+dir*30} ${210},${145+dir*30}`} fill="#ffaa00"/>
                </g>
              ))}

              {/* Bloch wave label */}
              <text x={300} y={220} textAnchor="middle" fill="#ffd700" fontSize={9} fontFamily="monospace">{"BLOCH KERNEL \u2022 \u03C8 = e^(ik\u00B7r) u(k,r)"}</text>
              <text x={300} y={235} textAnchor="middle" fill="#6c5ce7" fontSize={8} fontFamily="monospace">{"CASIMIR STABILIZED \u2022 Q \u2192 \u221E"}</text>
            </svg>

            <div style={{fontSize:8,color:C.dim,textAlign:"center",marginTop:8}}>
              Central strip from ARC_REACTOR_v7 · Interference + Casimir vacuum core · Provenance locked
            </div>
          </div>
