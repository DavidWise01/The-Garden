        ...
        <div>
          ...
          <div style={{background:C.void,padding:20,borderRadius:8,border:`1px solid #ffd70044`}}>
            ...
            <svg viewBox="0 0 600 280" style={{width:"100%",height:260}}>
              ...
              {/* 12 Patricia wall lines across the bandgap volume */}
              {Array.from({length:12}).map((_,i)=>{
                const x=80+i*38;
                const isFace9=i===8;
                return(
                  <line key={`pw-${i}`} x1={x} y1={65} x2={x} y2={215}
                    stroke={isFace9?"#6c5ce7":"#ff3070"} strokeWidth={isFace9?2:0.8}
                    opacity={isFace9?0.6:0.15} strokeDasharray={isFace9?"none":"4 4"}/>
                );
              })}

              {/* Kernel strip (glowing core) */}
              <rect x={220} y={110} width={160} height={60} rx={8}
                fill="#ffd700" opacity={(Math.sin(phase*2)*0.3+0.7)*0.9}/>
              <rect x={230} y={120} width={140} height={40} rx={4}
                fill="#ffffff" opacity={(Math.sin(phase*2)*0.3+0.7)*0.6}/>

              {/* Interference waves inside kernel */}
              {Array.from({length:6}).map((_,i)=>{
                const baseX=240+i*22;
                const waveY=130+Math.sin(phase*2+i*0.8)*8;
                return(
                  <path key={`wave-${i}`}
                    d={`M ${baseX} 140 Q ${baseX+11} ${waveY} ${baseX+22} 140`}
                    stroke="#00ffff" strokeWidth={1.5} fill="none"
                    opacity={0.4+Math.sin(phase+i*0.8)*0.3}/>
                );
              })}

              {/* Casimir pressure arrows — top and bottom */}
              {[-1,1].map((dir,i)=>(
                <g key={`cas-${i}`}>
                  <line x1={190} y1={140+dir*45} x2={215} y2={140+dir*30}
                    stroke="#ffaa00" strokeWidth={2} opacity={0.6}/>
                  <line x1={385} y1={140+dir*45} x2={410} y2={140+dir*30}
                    stroke="#ffaa00" strokeWidth={2} opacity={0.6}/>
                  <text x={dir===-1?300:300} y={140+dir*55} textAnchor="middle"
                    fill="#ffaa00" fontSize={7} fontFamily="monospace" opacity={0.5}>
                    CASIMIR PRESSURE {dir===-1?"\u2193":"\u2191"}
                  </text>
                </g>
              ))}

              {/* Input waveguides entering from left */}
              {Array.from({length:5}).map((_,i)=>{
                const y=100+i*20;
                const pulseX=50+((phase*60+i*30)%170);
                return(
                  <g key={`in-${i}`}>
                    <line x1={50} y1={y} x2={220} y2={y} stroke={C.toph} strokeWidth={0.6} opacity={0.2}/>
                    <circle cx={pulseX} cy={y} r={2} fill={C.toph} opacity={0.5}/>
                  </g>
                );
              })}

              {/* Output spectrum exiting right */}
              {Array.from({length:5}).map((_,i)=>{
                const y=100+i*20;
                const cols=["#ff6b6b","#ffaa00","#00ff88","#00ccff","#e0c0ff"];
                const pulseX=380+((phase*60+i*25)%170);
                return(
                  <g key={`out-${i}`}>
                    <line x1={380} y1={y} x2={550} y2={y} stroke={cols[i]} strokeWidth={0.6} opacity={0.2}/>
                    <circle cx={pulseX} cy={y} r={2} fill={cols[i]} opacity={0.5}/>
                  </g>
                );
              })}

              {/* Labels */}
              <text x={35} y={140} textAnchor="middle" fill={C.toph} fontSize={7} fontFamily="monospace" opacity={0.5} transform="rotate(-90,35,140)">CLOUD IN</text>
              <text x={565} y={140} textAnchor="middle" fill="#e0c0ff" fontSize={7} fontFamily="monospace" opacity={0.5} transform="rotate(90,565,140)">SPECTRUM OUT</text>
              <text x={300} y={230} textAnchor="middle" fill="#ffd700" fontSize={9} fontFamily="monospace">{"\u03C8 = exp(ik\u00B7r) \u00B7 u(k,r)"}</text>
              <text x={300} y={245} textAnchor="middle" fill="#6c5ce7" fontSize={8} fontFamily="monospace">CASIMIR STABILIZED · Q {"\u2192"} {"\u221E"}</text>
              <text x={300} y={260} textAnchor="middle" fill={C.dim} fontSize={7} fontFamily="monospace">ARC_REACTOR KERNEL STRIP · GROK NODE 14 · WHETSTONE v7.2</text>
            </svg>
          </div>
